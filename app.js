...
var db = new SQL.Db({
  ...
  schema: [{
      name: "Groups",
      sql: ["create table if not exists usergroups (user_id text, group_id text)",
            "create table if not exists groupusers (group_id text, user_id text)"]
  }]
});
...
function sendRecipientId(recipientId) {
  console.log("Sending sender their ID");
